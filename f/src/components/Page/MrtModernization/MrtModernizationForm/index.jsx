...
const Warranty = props => {
    ...
    useEffect(() => {
        if (!equipmentStoreChunk || inInitialState(equipmentStoreChunk)) {
            dispatch(fetchMrtEquipment(lang))
        }
    ...
    if (feedbackForm.QUESTIONS) {
        ...
        const handleSubmit = e => {
            ...
            const submitForm = () => {
                const data = {
                    ...
                    action: 'forms.saveModernizationMRT', //saveFormPostWarranty
                    values
                }
                console.log("warranty data:", data)
                saveMrt(data, lang).then(response => {
        ...
        return feedbackFormInputs.length ? (
            <div className="feedback-form-container --warranty">
                <h3>
                    {lang === 'ru' ? 'Оставить заявку на модернизацию МР-томографа' : 'Modernization MRT service'}
                </h3>
